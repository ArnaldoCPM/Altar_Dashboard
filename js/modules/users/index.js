...
function render() {
  if (!mount) return;
  const rows = users.map((user, index) => `<tr class="border-t text-sm"><td class="p-4">${escapeHtml(user.displayName)}</td><td class="p-4">${escapeHtml(user.email)}</td><td class="p-4">${escapeHtml(user.role)}</td><td class="p-4">${escapeHtml(user.chapelId || "—")}</td><td class="p-4">${statusLabel(user)}</td><td class="p-4 flex gap-3">${isCanonical(user) ? `<button data-edit-user="${index}" class="text-liturgical-blue font-bold">Editar</button>` : ""}${canSendAccess(user) ? `<button data-send-access="${index}" class="text-liturgical-blue font-bold">Enviar acesso</button>` : '<span class="text-xs text-slate-400">Acesso bloqueado</span>'}</td></tr>`).join("");
  mount.innerHTML = `<section class="space-y-6"><div class="flex justify-between items-center"><div><h2 class="text-2xl font-black text-slate-800">Usuários</h2><p class="text-sm text-slate-500">Administração de acessos ao SGSA.</p></div><button id="users-new" class="px-4 py-2 text-xs font-bold rounded-xl bg-liturgical-blue text-white">+ Novo usuário</button></div><section class="bg-white rounded-3xl border overflow-hidden"><table class="w-full text-left"><thead><tr class="bg-slate-50 text-xs text-slate-500"><th class="p-4">Nome</th><th class="p-4">E-mail</th><th class="p-4">Perfil</th><th class="p-4">Capela</th><th class="p-4">Estado</th><th class="p-4">Ações</th></tr></thead><tbody>${rows || '<tr><td colspan="6" class="p-8 text-center text-slate-500">Nenhum usuário cadastrado.</td></tr>'}</tbody></table></section></section>`;
