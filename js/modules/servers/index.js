...
import { createServer, deleteServer } from "../../data/servers.js";
...
let unsubscribe = null, data = [], chapels = [], mount = null;
...
const scoped = (payload, existing=null) => canAccessAdminMode() ? {...payload, chapelId: payload.chapelId ?? chapels.find(c=>c.name===payload.Capela)?.id ?? existing?.chapelId ?? null} : {...payload, chapelId:getCurrentChapelId() ?? existing?.chapelId ?? null, Capela:getCurrentProfile()?.chapelName ?? existing?.Capela ?? payload.Capela};
function render() {
    if (!mount) return;
    const q = document.getElementById('servers-search').value.toLowerCase();
    const chapel = document.getElementById('servers-filter-chapel').value;
    const state = document.getElementById('servers-filter-state').value;
    const allergy = document.getElementById('servers-filter-allergy').value;
    const type = document.getElementById('servers-filter-type').value;
    const filtered = data
    ...
    filtered.forEach((server) => {
        ...
        const candidate = server.Whatsapp_candidato
            ...
            : '—';
        row.innerHTML = `<td class="p-4"><b>${server.Nome || 'Sem nome'}</b><div class="text-xs text-slate-400">${server.id}</div></td><td class="p-4">${age(server) ?? '—'} anos<br><span class="text-xs">${server.Sexo || '—'}</span></td><td class="p-4">${server.Capela || 'S/D'}</td><td class="p-4">${candidate}</td><td class="p-4">${canEdit(server) ? '<button data-edit class="text-amber-600 font-bold">Editar</button>' : ''} ${canDelete(server) ? '<button data-delete class="text-rose-600 font-bold ml-2">Apagar</button>' : ''}</td>`;
...
export async function initialize({mountElement}){destroy();mount=document.createElement('div');mount.dataset.moduleRoot='servers';mountElement.append(mount);chapels=await getActiveChapels();renderServersWorkspace(mount,{isAdmin:canAccessAdminMode(),canCreate:canCreateServer(getCurrentChapelId())});const chapel=document.getElementById('servers-filter-chapel');chapels.forEach(c=>chapel.add(new Option(c.name,c.name)));['servers-search','servers-filter-chapel','servers-filter-state','servers-filter-allergy','servers-filter-type'].forEach(id=>document.getElementById(id).addEventListener(id==='servers-search'?'input':'change',render));document.getElementById('servers-add')?.addEventListener('click',openNew);bindModal();document.getElementById('servers-import-toggle')?.addEventListener('click',()=>document.getElementById('servers-import-panel').classList.toggle('hidden'));document.getElementById('servers-csv-file')?.addEventListener('change',e=>{const file=e.target.files[0];if(!file||!canAccessAdminMode())return;const reader=new FileReader();reader.onload=()=>{const items=csvToObjects(reader.result);if(!items?.length)return error('O formato do ficheiro CSV não foi reconhecido.');document.dispatchEvent(new CustomEvent('shell:confirm',{detail:{title:'Sincronizar a base de dados',message:`Foram detetados ${items.length} servidores válidos. Deseja continuar?`,onConfirm:async()=>{const bar=document.getElementById('servers-upload-progress'),fill=document.getElementById('servers-upload-progress-bar');bar.classList.remove('hidden');try{await importServers(items,scoped,p=>fill.style.width=p+'%');e.target.value=''}catch(err){error('Ocorreu um erro ao carregar os lotes: '+err.message)}finally{bar.classList.add('hidden')}}}}))};reader.readAsText(file,'UTF-8')});unsubscribe=onSnapshot(buildServersQuery(),snap=>{data=snap.docs.map(d=>({id:d.id,...d.data()}));render()},err=>error('Erro ao carregar servidores: '+err.message));}
export function destroy(){unsubscribe?.();unsubscribe=null;document.getElementById('server-form')&&(document.getElementById('server-form').onsubmit=null);document.getElementById('edit-server-modal')?.classList.add('hidden');mount?.remove();mount=null;data=[];}
