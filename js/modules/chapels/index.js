...
function duplicateName(name, currentId = null) { return chapels.some((chapel) => chapel.id !== currentId && normalizeChapelName(chapel.name) === normalizeChapelName(name)); }
function render() {
  if (!mount) return;
  const search = mount.querySelector("[data-chapel-search]")?.value || "";
...
function openForm(chapel = null) {
  ...
  dialog.className = "rounded-2xl border p-0 shadow-xl";
  dialog.innerHTML = `<form method="dialog" class="w-[min(92vw,36rem)] space-y-4 p-6"><div><h3 class="text-xl font-bold">${chapel ? "Editar capela" : "Nova capela"}</h3><p class="mt-1 text-xs text-slate-500">Observações devem ser administrativas e não conter dados sensíveis.</p></div><label class="block text-sm">Nome<input required name="name" maxlength="120" value="${escapeHtml(chapel?.name || "")}" class="mt-1 w-full rounded border p-2"></label><label class="block text-sm">Endereço<input name="address" maxlength="250" value="${escapeHtml(chapel?.address || "")}" class="mt-1 w-full rounded border p-2"></label><label class="block text-sm">Observações<textarea name="notes" maxlength="500" class="mt-1 w-full rounded border p-2">${escapeHtml(chapel?.notes || "")}</textarea></label><label class="flex gap-2 text-sm"><input type="checkbox" name="active" ${chapel?.active !== false ? "checked" : ""}> Capela ativa</label><p data-form-error class="hidden text-sm text-rose-600"></p><div class="flex justify-end gap-3"><button value="cancel" class="rounded border px-4 py-2">Cancelar</button><button value="save" class="rounded bg-emerald-600 px-4 py-2 font-bold text-white">Salvar</button></div></form>`;
  document.body.append(dialog); dialog.showModal();
  dialog.querySelector("form").addEventListener("submit", async (event) => { if (event.submitter?.value !== "save") return; event.preventDefault(); const form = new FormData(event.currentTarget); const payload = { name: form.get("name"), address: form.get("address"), notes: form.get("notes"), active: form.get("active") === "on" }; const error = dialog.querySelector("[data-form-error]"); if (duplicateName(payload.name, chapel?.id)) { error.textContent = "Já existe uma capela com este nome."; error.classList.remove("hidden"); return; } try { let renameResult = null; if (!chapel) await createChapel(payload); else if (normalizeChapelName(payload.name) !== normalizeChapelName(chapel.name)) { const impact = await countServersByChapel(chapel.id); const confirmed = window.confirm(`Renomear esta capela atualizará o campo Capela de ${impact} servidor(es). Registros históricos não serão alterados. Deseja continuar?`); if (!confirmed) return; renameResult = await renameChapelAndSyncServers(chapel.id, payload); } else await updateChapel(chapel.id, payload); if (renameResult) successMessage = `Capela renomeada e ${renameResult.syncedServers} servidor(es) atualizado(s).`; dialog.close(); await refresh(); } catch (err) { const progress = err.syncProgress; const chapelProgress = err.chapelUpdateProgress; error.textContent = progress ? `A sincronização foi interrompida após ${progress.completed} de ${progress.total} servidores. ${err.message}` : chapelProgress ? `${chapelProgress.completed} de ${chapelProgress.total} servidores foram atualizados, mas não foi possível concluir a atualização da capela: ${err.message}` : err.message; error.classList.remove("hidden"); } });
