...
function renderPolesSummary(poles, summary) {
    ...
    const items = poles.map((pole) => `<li class="flex items-center justify-between gap-3"><span>${escapeHtml(pole.name)}</span>${statusBadge("pole", pole.active ? "active" : "inactive")}</li>`).join("");
...
function renderDetails(formation, permissions, ui, poles, poleSummary) {
    ...
    return `<section class="mx-auto max-w-3xl rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"><div class="flex flex-wrap items-start justify-between gap-4"><div><div class="flex flex-wrap items-center gap-2"><h2 class="text-2xl font-bold text-slate-800">Detalhes da formação</h2>${statusBadge("formation", formation.status)}</div><p class="mt-1 text-sm text-slate-500">${escapeHtml(formation.name)}</p></div><div class="flex flex-wrap gap-2"><button class="rounded-lg border border-slate-300 px-3 py-2 text-sm font-semibold" data-formation-action="back">Voltar</button><button class="rounded-lg bg-slate-800 px-3 py-2 text-sm font-bold text-white" data-formation-action="poles" data-formation-id="${formation.id}">${poleSummary?.status === "loaded" && !poles.length ? "Configurar primeiro polo" : "Gerenciar polos"}</button>${permissions.canEdit ? `<button class="rounded-lg border border-slate-300 px-3 py-2 text-sm font-semibold" data-formation-action="edit" data-formation-id="${formation.id}">Editar</button>` : ""}</div></div><div class="mt-4">${renderFeedback(ui)}</div><dl class="mt-6 grid gap-4 text-sm sm:grid-cols-2"><div><dt class="font-semibold text-slate-500">Descrição</dt><dd class="mt-1 text-slate-800">${escapeHtml(formation.description || "Não informada")}</dd></div><div><dt class="font-semibold text-slate-500">Etapa</dt><dd class="mt-1 text-slate-800">${STAGE_LABELS[formation.stage]}</dd></div><div><dt class="font-semibold text-slate-500">Modalidades</dt><dd class="mt-1 text-slate-800">${(formation.modalities || []).map((modality) => MODALITY_LABELS[modality]).join(", ")}</dd></div><div><dt class="font-semibold text-slate-500">Período</dt><dd class="mt-1 text-slate-800">${formatDate(formation.startDate)} — ${formatDate(formation.endDate)}</dd></div><div><dt class="font-semibold text-slate-500">Criado por</dt><dd class="mt-1 text-slate-800">${escapeHtml(formation.createdBy || "Não informado")}</dd></div><div><dt class="font-semibold text-slate-500">Criado em</dt><dd class="mt-1 text-slate-800">${formatDate(formation.createdAt)}</dd></div><div><dt class="font-semibold text-slate-500">Atualizado em</dt><dd class="mt-1 text-slate-800">${formatDate(formation.updatedAt)}</dd></div></dl>${renderPolesSummary(poles, poleSummary)}${transitions ? `<section class="mt-6 border-t border-slate-200 pt-4"><h3 class="text-sm font-semibold text-slate-700">Ações de status</h3><div class="mt-3 flex flex-wrap gap-2">${transitions}</div></section>` : ""}</section>`;
}
