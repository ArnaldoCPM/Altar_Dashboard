const STAGE_LABELS = {
    first: "1ª Fase — 6 a 11 anos",
    second: "2ª Fase — 12 a 24 anos"
};
...
function statusActions(formation, canEdit) {
    ...
    return transitions[formation.status]
        .filter((status) => status !== formation.status)
        .map((status) => `<button class="rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-50" data-formation-action="status" data-formation-id="${formation.id}" data-formation-status="${status}">${STATUS_LABELS[status]}</button>`)
        .join("");
}
