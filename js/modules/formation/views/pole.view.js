...
function renderPoleList(state, actions) {
    ...
    const rows = poles.map((pole) => `<tr class="border-t border-slate-100"><td class="px-4 py-3 font-semibold text-slate-800">${escapeHtml(pole.name)}</td><td class="px-4 py-3 text-slate-600">${escapeHtml(actions.chapelNameById[pole.baseChapelId] || pole.baseChapelId)}</td><td class="px-4 py-3 text-slate-600">${(pole.chapelIds || []).length}</td><td class="px-4 py-3 text-slate-600">${(pole.coordinatorIds || []).length}</td><td class="px-4 py-3">${statusBadge("pole", pole.active ? "active" : "inactive")}</td><td class="px-4 py-3"><div class="flex flex-wrap gap-2"><button class="rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-semibold text-slate-700" data-pole-action="encounters" data-pole-id="${pole.id}">Gerenciar encontros</button>${editablePoleIds.has(pole.id) ? `<button class="rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-semibold text-slate-700" data-pole-action="edit" data-pole-id="${pole.id}">Editar</button>` : ""}${actions.canTogglePole ? `<button class="rounded-lg border px-3 py-1.5 text-xs font-semibold ${pole.active ? "border-rose-200 text-rose-700 hover:bg-rose-50" : "border-slate-300 text-slate-700 hover:bg-slate-50"}" data-pole-action="toggle" data-pole-id="${pole.id}" data-pole-active="${pole.active}">${pole.active ? "Desativar" : "Ativar"}</button>` : ""}</div></td></tr>`).join("");
