...
function renderPoleList(state, actions) {
    ...
    const rows = poles.map((pole) => `<tr class="border-t border-slate-100"><td class="whitespace-nowrap px-4 py-3 font-semibold text-slate-800">${escapeHtml(pole.name)}</td><td class="px-4 py-3 text-slate-600">${escapeHtml(actions.chapelNameById[pole.baseChapelId] || pole.baseChapelId)}</td><td class="whitespace-nowrap px-4 py-3 text-slate-600">${(pole.chapelIds || []).length}</td><td class="whitespace-nowrap px-4 py-3 text-slate-600">${(pole.coordinatorIds || []).length}</td><td class="whitespace-nowrap px-4 py-3">${statusBadge("pole", pole.active ? "active" : "inactive")}</td><td class="whitespace-nowrap px-4 py-3"><div class="flex flex-wrap gap-2"><button class="rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-semibold text-slate-700 focus:outline-none focus:ring-2 focus:ring-sky-500 focus:ring-offset-2" data-pole-action="encounters" data-pole-id="${pole.id}">Gerenciar encontros</button>${editablePoleIds.has(pole.id) ? `<button class="rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-semibold text-slate-700 focus:outline-none focus:ring-2 focus:ring-sky-500 focus:ring-offset-2" data-pole-action="edit" data-pole-id="${pole.id}">Editar</button>` : ""}${actions.canTogglePole ? `<button class="rounded-lg border px-3 py-1.5 text-xs font-semibold focus:outline-none focus:ring-2 focus:ring-sky-500 focus:ring-offset-2 ${pole.active ? "border-rose-200 text-rose-700 hover:bg-rose-50" : "border-slate-300 text-slate-700 hover:bg-slate-50"}" data-pole-action="toggle" data-pole-id="${pole.id}" data-pole-active="${pole.active}">${pole.active ? "Desativar" : "Ativar"}</button>` : ""}</div></td></tr>`).join("");
    const content = state.ui.loading ? '<p class="p-6 text-sm text-slate-500">Carregando polos...</p>' : poles.length ? `<div class="overflow-x-auto"><table class="min-w-[820px] text-sm"><thead class="bg-slate-50 text-left text-xs uppercase tracking-wide text-slate-500"><tr><th scope="col" class="px-4 py-3">Nome</th><th scope="col" class="px-4 py-3">Capela base</th><th scope="col" class="px-4 py-3">Capelas</th><th scope="col" class="px-4 py-3">Coordenadores</th><th scope="col" class="px-4 py-3">Status</th><th scope="col" class="px-4 py-3">Ações</th></tr></thead><tbody>${rows}</tbody></table></div>` : '<p class="p-6 text-sm text-slate-500">Nenhum polo cadastrado.</p>';
    return `<section class="space-y-6"><div class="flex flex-wrap items-center justify-between gap-3"><div><h2 class="text-2xl font-bold text-slate-800">Polos</h2><p class="mt-1 text-sm text-slate-500">${escapeHtml(currentFormation?.name || "Formação")}</p></div><div class="flex flex-wrap gap-2"><button class="rounded-lg border border-slate-300 px-4 py-2 text-sm font-semibold" data-pole-action="back">Voltar</button>${actions.canCreatePole ? '<button class="rounded-xl bg-emerald-600 px-4 py-2 text-sm font-bold text-white" data-pole-action="create">Novo polo</button>' : ""}</div></div>${renderFeedback(state.ui)}<div class="rounded-2xl border border-slate-200 bg-white shadow-sm">${content}</div></section>`;
}
