...
async function getPoles(formationId) {
    const snapshot = await getDocs(query(polesCollection(formationId), orderBy("createdAt", "desc")));
    return snapshot.docs.map(toPole);
}
